...
pairs in Thiemann's completeness proof
\cite{thiemann_2009_outermost}, we use the complete annotated
dependency-pair (ADP) framework for innermost AST
\cite{kassing_giesl_2026_adp} to extract a non-converging computation of a
canonical shape.

The classical dependency-pair method and its modular processor architecture
are developed in \cite{arts_giesl_2000_dependency_pairs,giesl2006mechanizing}.
All ADP definitions and processor results invoked below are due to Kassing and
Giesl \cite{kassing_giesl_2026_adp}.  The contribution of this thesis is their
application to the particular transformed system \(\P'\), including the
dependency-graph analysis, orientation argument, and subsequent extraction of
an outermost source computation.
...
We briefly record only the fragment of the ADP framework needed below.  For
every defined symbol \(h\), let \(h^{\sharp}\) be a fresh annotated copy.  If \(r\) is a
...
An ADP chain tree is an RST-like, finitely branching tree over annotated terms
in which distinguished annotated steps track potentially infinite calls; every
infinite branch must contain infinitely many such steps.  Operationally, a step
at an annotation
